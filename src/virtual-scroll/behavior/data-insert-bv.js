            ...
            Promise.all([nextDataPromise, setLoadingPromise])
                ...
                setLoadingPromise.then(() => {
                    ...
                    if (!nextList || nextList.length === 0) {
                        return this._setDataWrapper({ isLoadingNextDone: true, isLoadingNext: false });
                    }
                ...
                .catch(() => this._setDataWrapper({ isLoadingNext: false }));
        },
        _doWhenScroll2Top() {
            ...
            Promise.all([nextDataPromise, setLoadingPromise])
                ...
                setLoadingPromise.then(() => {
                    // 如果没有返回新的数据，直接返回
                    if (!nextList || nextList.length === 0) {
                        return this._setDataWrapper({ isLoadingHistoryDone: true, isLoadingHistory: false });
                    }
                ...
                .catch(() => this._setDataWrapper({ isLoadingHistory: false }));
        },
    },
});
