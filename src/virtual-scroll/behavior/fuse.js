import {
  cloneGroup, generateGroups, generateNewMsgGroup,
  getEndElement, getGroupIds, getIndexWithGroupId,
  processData, setItemGroupId
} from '../utils/util'
...
export default class Fuse {
  ...
  getClearingData() {
    const {
      listenedGroupIds,
      data: {currentGroupId, renderedHistoryGroups, renderedNextGroups}
    ...
    const filterGroupId = groupId => !currentRenderedGroupIds.includes(groupId)
    const clearingGroupIds = listenedGroupIds.filter(filterGroupId)
  ...
  insertListByScrollType(list, scrollType) {
    ...
    const keyInfo = this.getKeyInfoByScrollType(scrollType)
    const {
      groupsRepo, renderedGroup, renderKey, endGroupInRepo, endGroupInRendered
    } = keyInfo
  ...
  getKeyInfo(groupId) {
    ...
    const indexInRender = this.getIndexWithGroupIdInRendered(groupId)
    return {
      renderKeyName, groupsRepo, indexInRepo, renderedGroups, indexInRender
    }
  }
  ...
  getNextRenderData(currentGroupId) {
    ...
    const {
      renderKeyName, groupsRepo, indexInRepo, renderedGroups, indexInRender
    } = this.getKeyInfo(currentGroupId)
